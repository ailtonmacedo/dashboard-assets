...
Arquivos gerados:
  Os arquivos serão salvos dentro de uma pasta com o formato:
  DD-MM-AAAA_HH-MM-SS

  Exemplo:
  08-07-2026_20-30-15/painel_etfs.html
  08-07-2026_20-30-15/asset_correlation.png
  08-07-2026_20-30-15/rolling_asset_correlation.png
...
def criar_diretorio_execucao(base_dir=".") -> str:
    """
    Cria um diretório para salvar os arquivos gerados nesta execução.
    ...
    Exemplo:
      08-07-2026_20-30-15
    """
    nome_diretorio = datetime.now().strftime("%d-%m-%Y_%H-%M-%S")
    caminho_diretorio = os.path.abspath(os.path.join(base_dir, nome_diretorio))
...
def analisar(ticker: str) -> dict:
    ...
    macd_hist = (macd - sinal_macd).iloc[-1]
...
<dl>
<dt>RSI 14</dt>
<dd>Força do movimento nos últimos 14 dias, de 0 a 100. Acima de 70 = sobrecomprado. Abaixo de 30 = sobrevendido.</dd>

<dt>MM 9x21</dt>
<dd>Cruzamento das médias móveis de 9 e 21 dias. &uarr; sim = tendência de alta. &darr; não = tendência de baixa.</dd>

<dt>MACD</dt>
<dd>Mede se a tendência está ganhando ou perdendo força. Positivo = momento comprador. Negativo = momento vendedor.</dd>

<dt>Bollinger</dt>
<dd>Mostra onde o preço está dentro da faixa normal de oscilação. Banda inferior = preço baixo. Banda superior = preço alto. Meio = dentro do normal.</dd>

<dt>Vol. rel.</dt>
<dd>Volume do dia dividido pela média de 21 dias. Acima de 1,5 = movimento com força. Abaixo de 1 = movimento fraco.</dd>

<dt>Pontos</dt>
<dd>Soma dos quatro indicadores, de &minus;4 a +4. A partir de +2 vira COMPRA. A partir de &minus;2 vira VENDA. No meio fica NEUTRO.</dd>
</dl>
</div>

<p class="nota">
RSI em vermelho = sobrecomprado (&gt;70), verde = sobrevendido (&lt;30).
Volume relativo em negrito quando &ge; 1,5x a média.
...
def gerar_heatmap(retornos, caminho="asset_correlation.png", mostrar=False):
    ...
    plt.title(
        "Correlação entre retornos diários (log)",
        fontsize=13,
        pad=14,
    )

    legenda = (
        "Como ler: perto de +1 = os dois sobem e caem juntos, pouca "
        "diversificação • perto de −1 = um sobe quando o outro cai, "
        "se protegem mutuamente • perto de 0 = movem-se de forma independente"
    )
...
def gerar_rolling(retornos, caminho="rolling_asset_correlation.png", mostrar=False):
    ...
    plt.title(
        f"Correlação móvel de {JANELA_ROLLING} dias",
        fontsize=13,
        pad=12,
    )
    ...
    legenda = (
        ...
        "(> 0,5) = andam fortemente juntos • faixa vermelha (< −0,5) = "
        "se protegem mutuamente • linha cruzando o zero = mudança de regime"
    )
...
def analise_correlacao(diretorio_saida, mostrar=False):
...
def main():
    ...
    diretorio_saida = criar_diretorio_execucao()
    ...
    print(f"Painel visual gerado: {caminho}")
    ...
    if (tabela.get("Fonte") == "FALHOU").any():
        print(
            ...
            "\n 3. Teste a conexão: Yahoo pode bloquear redes corporativas ou VPNs."
        )

    print(
        "\nLembrete: cruze com o viés macro antes de qualquer decisão."
        "\nFerramenta educativa — não é recomendação de investimento."
    )
